...
export function PostItem({ post: initialPost, compact = false }: { post: PostType, compact?: boolean }) {
  const { user } = useAuth()
  ...
  const toggleLike = async () => {
    ...
    if (!wasLiked) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
  ...
  const toggleRepost = async () => {
    ...
    if (!wasReposted) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
  ...
  const toggleBookmark = async () => {
    ...
    if (!wasBookmarked) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
  ...
  const toggleHideLikes = async () => {
    if (!user || !post) return
    const newHide = !post.settings?.hide_likes
    const newSettings = { ...(post.settings || {}), hide_likes: newHide }
    setPost(p => ({ ...p, settings: newSettings }))
    await supabase.from('posts').update({ settings: newSettings }).eq('id', post.id)
    showToast(newHide ? 'Likes are now hidden from others.' : 'Likes are now visible to everyone.', 'success')
  }
  ...
  const handlePostOptions = () => {
    ...
    if (user.id === post.creator_id) {
      showActionSheet('Post Options', [
        { text: post.settings?.hide_likes ? 'Show Likes Count' : 'Hide Likes Count', icon: post.settings?.hide_likes ? 'eye' : 'eye-off', onPress: toggleHideLikes },
    ...
    <View style={styles.post}>
      ...
      <View style={styles.actions}>
        <TouchableOpacity style={styles.actionBtn} onPress={toggleLike} activeOpacity={0.7}>
          ...
          />
          {(post.likes_count || 0) > 0 && (!post.settings?.hide_likes || user?.id === post.creator_id) && (
