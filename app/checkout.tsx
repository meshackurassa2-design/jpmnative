...
export default function CheckoutScreen() {
  ...
  const handleCheckout = async () => {
    ...
    setPaymentStatusMsg('Initiating payment...')
    try {
      // Ensure phone starts with 255 for ClickPesa
      let formattedPhone = phone.replace(/\D/g, '')
      if (formattedPhone.startsWith('0')) {
        formattedPhone = '255' + formattedPhone.slice(1)
      } else if (formattedPhone.length === 9 && !formattedPhone.startsWith('255')) {
        formattedPhone = '255' + formattedPhone
      }

      // 1. Process ClickPesa Payment
      await initiateAndWait(
        finalTotal,
        formattedPhone,
        user.id,
        'Dapaz App Order',
        {
          onStatus: (status) => {
            if (status === 'PROCESSING') setPaymentStatusMsg('Please check your phone and enter your PIN.')
            else if (status === 'SUCCESS' || status === 'SETTLED') setPaymentStatusMsg('Payment successful, finalizing order...')
          }
        }
      )

      // 2. ClickPesa succeeded, now process our order in Supabase
      ...
      // 4. Decrease Inventory and update Shops JSON
      try {
        for (const item of items) {
           const { data: invData } = await supabase.from('shop_inventory').select('quantity').eq('id', item.id).single();
           if (invData && typeof invData.quantity === 'number') {
              const newQty = Math.max(0, invData.quantity - (item.quantity || 1));
              await supabase.from('shop_inventory').update({ quantity: newQty }).eq('id', item.id);
              
              const { data: shopData } = await supabase.from('shops').select('products').eq('id', item.shopId).single();
              if (shopData && shopData.products) {
                 const updatedProds = shopData.products.map((p: any) => {
                    if (p.id === item.id) {
                       return { ...p, quantity: newQty, out_of_stock: newQty === 0 };
                    }
                    return p;
                 });
                 await supabase.from('shops').update({ products: updatedProds }).eq('id', item.shopId);
              }
           }
        }
      } catch (e) {
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }} showsVerticalScrollIndicator={false}>
          <View style={[styles.receiptPaper, { backgroundColor: '#0A0A0A', borderColor: '#222222', alignItems: 'center', width: '100%', marginBottom: 24 }]}>
            ...
            <View style={{ width: '100%', borderTopWidth: 1, borderTopColor: '#333', paddingTop: 20 }}>
              <Text style={{ color: '#fff', fontSize: 18, fontWeight: 'bold', marginBottom: 16 }}>Order Details</Text>
              
              <Text style={{ color: '#aaa', fontSize: 14, marginBottom: 4 }}><Text style={{ color: '#fff', fontWeight: 'bold' }}>Name:</Text> {name}</Text>
              <Text style={{ color: '#aaa', fontSize: 14, marginBottom: 4 }}><Text style={{ color: '#fff', fontWeight: 'bold' }}>Phone:</Text> +255 {phone}</Text>
              <Text style={{ color: '#aaa', fontSize: 14, marginBottom: 4 }}><Text style={{ color: '#fff', fontWeight: 'bold' }}>Location:</Text> {address}, {city}</Text>
              <Text style={{ color: '#aaa', fontSize: 14, marginBottom: 16 }}><Text style={{ color: '#fff', fontWeight: 'bold' }}>Date:</Text> {new Date().toLocaleDateString()}</Text>

              <Text style={{ color: '#fff', fontSize: 18, fontWeight: 'bold', marginBottom: 16 }}>Items Receipt</Text>
              {orderedItems.map((item, idx) => (
                <View key={idx} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 12 }}>
                  <Text style={{ color: '#aaa', flex: 1, paddingRight: 10 }}>{item.quantity || 1}x {item.name} <Text style={{ color: '#888', fontStyle: 'italic' }}>({item.shopName || 'Shop'})</Text></Text>
                  ...
                  <html>
                    ...
                    <body>
                      ...
                      <div style="margin-bottom: 30px; padding: 15px; background: #f9f9f9; border-radius: 8px;">
                        ...
                        <p><strong>Phone:</strong> +255 ${phone}</p>
                      ...
                      ${orderedItems.map(item => `
                        <div class="item">
                          <span>${item.quantity || 1}x ${item.name} <em>(${item.shopName || 'Shop'})</em></span>
                      ...
                      <p style="text-align: center; margin-top: 40px; color: #888;">Generated by Dapaz App</p>
                    </body>
                  </html>
                `;
  ...
  return (
    <View style={[styles.container, { paddingTop: topPadding, paddingBottom: insets.bottom || 24 }]}>
      ...
      <View style={styles.footer}>
        {isProcessing && paymentStatusMsg ? (
           <Text style={styles.paymentStatusText}>{paymentStatusMsg}</Text>
        ) : null}
        <TouchableOpacity 
          ...
          {isProcessing ? (
            ...
            <Text style={styles.checkoutBtnText}>
               Pay TSH {finalTotal.toLocaleString()}
            </Text>
          )}
        </TouchableOpacity>
      </View>
