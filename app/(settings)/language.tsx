...
export default function LanguageScreen() {
  ...
  const toggleLang = (code: string) => {
    const isCurrentlySelected = !!selected[code];
    const newSelectedState = !isCurrentlySelected;
    
    const next = { ...selected, [code]: newSelectedState };
    setSelected(next);
    
    if (newSelectedState) {
      setLang(code);
    } else {
      const anyOn = Object.keys(next).find(k => next[k]);
      if (anyOn) setLang(anyOn);
      else setLang('en');
    }
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <Text style={styles.subtitle}>You'll be able to see posts, people, and trends in any languages you choose.</Text>

        <View style={styles.list}>
          {LANGUAGES.map(l => (
            <View key={l.code} style={styles.item}>
              ...
              <Switch 
                value={!!selected[l.code]} 
                onValueChange={() => toggleLang(l.code)}
                trackColor={{ false: '#333', true: '#fff' }}
                thumbColor={!!selected[l.code] ? '#000' : '#fff'}
