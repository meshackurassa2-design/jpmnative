...
const LANGUAGES = [
  ...
  { code: 'cha', label: 'Chagga (Vunjo)', native: 'Kivunjo' },
...
export default function WelcomeLanguageScreen() {
  ...
  const handleContinue = () => {
    // Navigate to the login screen after choosing language
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <Text style={styles.subtitle}>{t('select_language_sub')}</Text>
        ...
        <View style={styles.list}>
          {LANGUAGES.map(l => (
            <View key={l.code} style={styles.item}>
              ...
              <Switch 
                value={selectedLang === l.code} 
                onValueChange={() => toggleLang(l.code)}
                trackColor={{ false: '#333', true: '#fff' }}
                thumbColor={selectedLang === l.code ? '#000' : '#fff'}
