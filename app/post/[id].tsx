...
export default function PostDetail() {
  ...
  const handleReplyToComment = (item: any) => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)
    const username = item.profiles?.username || 'user'
    const parentId = item.parent_id || item.id
    setReplyingToComment({ ...item, id: parentId, replyToUsername: username })
    setNewComment(prev => {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`post_detail_${postId}_${Date.now()}`)
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'comments', filter: `post_id=eq.${postId}` }, (payload) => {
        setComments(prev => prev.map(c => c.id === payload.new.id ? { ...c, ...payload.new, profiles: c.profiles, is_liked: c.is_liked } : c))
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'posts', filter: `id=eq.${postId}` }, (payload) => {
        setPost(p => p && p.id === payload.new.id ? { ...p, ...payload.new, profiles: p.profiles, is_liked: p.is_liked, is_reposted: p.is_reposted } : p)
      })
      .subscribe()
  ...
  const toggleRepost = async () => {
    ...
    if (!isReposted) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
  ...
  const toggleCommentLike = async (commentId: string, currentLiked: boolean) => {
    if (!user) return
    if (!currentLiked) Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
  ...
  const submitComment = async () => {
    ...
    const { data, error } = await supabase.from('comments').insert({
      post_id: post.id, user_id: user.id, content: newComment.trim(), parent_id: replyingToComment ? replyingToComment.id : null
    ...
    <View style={{ backgroundColor: colors.background }}>
      {replyingToComment && (
        <View style={styles.replyBanner}>
          <Text style={styles.replyBannerText} numberOfLines={1}>
            Replying to @{replyingToComment.replyToUsername || replyingToComment.profiles?.username || 'user'}
          </Text>
          <TouchableOpacity onPress={() => setReplyingToComment(null)} style={{ padding: 4 }}>
      ...
      <View style={styles.inputArea}>
        ...
        <TextInput
          ref={commentInputRef}
          style={styles.input}
          placeholder={replyingToComment ? `Reply to @${replyingToComment.replyToUsername || 'comment'}...` : "Add a comment..."}
  ...
  const renderCommentItem = (item: any, isReply = false) => (
    <View style={[styles.commentRow, isReply && styles.replyRowContainer]} key={item.id}>
      {item.profiles?.avatar_url ? (
        <Image source={{ uri: getCdnUrl(item.profiles.avatar_url) }} style={[styles.commentAvatar, isReply && styles.replyAvatar]} />
      ) : (
        <View style={[styles.commentAvatar, styles.avatarFallback, isReply && styles.replyAvatar]}>
          <Text style={[styles.avatarFallbackText, isReply && { fontSize: 11 }]}>{item.profiles?.full_name?.[0] || '?'}</Text>
        </View>
      )}
      <View style={styles.commentContent}>
        ...
        <View style={styles.commentFooter}>
          ...
          <TouchableOpacity onPress={() => handleReplyToComment(item)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
  ...
  const renderComment = ({ item }: { item: any }) => {
    const childReplies = comments.filter(c => c.parent_id === item.id)
    return (
      <View>
